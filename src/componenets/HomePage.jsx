...
export default function Homepage() {
    ...
    const arr2 = ["סֻכָּה", "בֵּיצָה", "רֹאשׁ", "הַשָּׁנָה", "תַּעֲנִית", "מְגִלָּה", "מוֹעֵד", "קָטָן"];
    ...
    const allArrs = ["שבת","ברכות"]
    // arr1.concat(arr2, arr3, arr4, arr5, arr6, arr7); 
    
    
    const [selectedGmara,setSelectedGmara]= useState(null);
    const handle = () => {
        // Your click handling function
    }
        ...
        return arrays.map((arr, index) => (
            <div key={index} className="card">
                <div className="flex flex-wrap justify-content-center gap-3 mb-4">
                    {arr.map((e, i) => (
                        <Button rounded outlined aria-label="Filter" onClick={handle} key={i}>{e}</Button>
                    ))}
                </div>
                <br />
            </div>
